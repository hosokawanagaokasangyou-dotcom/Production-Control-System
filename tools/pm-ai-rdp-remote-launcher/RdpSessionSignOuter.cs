...
internal static class RdpSessionSignOuter
{
    internal static bool TrySignOutCurrentSession(out string? errorMessage)
    {
        errorMessage = null;
        ...
        LauncherLog.Info("サインアウト対象セッション ID=" + sessionId + " (自プロセス PID=" + Environment.ProcessId + ")");

        if (WTSApi.WTSLogoffSession(WTSApi.WtsCurrentServerHandle, (int)sessionId, false))
        {
            LauncherLog.Info("WTSLogoffSession でセッションをサインアウトしました");
            return true;
        }
    ...
    private static bool TryShutdownLogoffFallback(out string? errorMessage)
    {
        ...
        try
        {
            ...
            LauncherLog.Info("shutdown.exe /l /f を起動してサインアウトを試行: " + shutdownPath);
