...
配台システム（PMD）のリモートデスクトップタブと、接続先 PC 上の C# ランチャー（PmAiRdpRemoteLauncher）の関係・設定・ログイン・引数を一覧する。
...
| exe | 役割 | 動く場所 | 中身 |
|-----|------|----------|------|
| **PMD.exe** | 配台システム本体（リモートデスクトップタブ含む） | 操作者 PC（ローカル） | C# スタブ → Java `PmAiFxApp` |
| **PmAiRdpRemoteLauncher.exe** | 接続先で RPA を起動するランチャー | RDP 先 PC（リモートセッション） | C# 単体（.NET） |

**廃止**: **PmAiRpaLuncher.exe**（`RemoteDesktopFxApp` 専用シェル）は単体配布を終了。操作者 PC は **PMD.exe → リモートデスクトップタブ** を使用する。

旧名 **PmAiRdpLauncher.exe** は配台 PMD と混同しやすいため改名済み。

---

## 2. リモートデスクトップ UI（配台 PMD 内）

`RequestFormRemoteDesktopPane` をメインシェルタブ「リモートデスクトップ」で表示する。

| 項目 | 配台システム（PMD） |
|------|---------------------|
| エントリクラス | `PmAiFxApp` |
| 表示タイトル | 配台システム |
| シェル UI | `MainShell`（リモートデスクトップはその一タブ） |
| PC ローカル設定ルート | `%USERPROFILE%\.pm-ai-desktop` |
| 操作者 bin | `factory-operator-users.bin`（サマリ Excel 同階層） |
| ランチャー配備先の既定 | サマリ Excel（`summary_ai_dispatch.xlsx`）の親フォルダ |
...
\\192.168.0.101\共有フォルダ\掲示板\rpa_luncher\
  ...
  launcher-yyyyMMdd.log              … 接続先ランチャーの日次ログ
  DATA\
...
| **launcher-yyyyMMdd.log** | UI でプレビュー可能 | 自身が書き込み |
