...
"@
    ...
    if ($BundleKind -eq 'InitialInstall') {
        $rmLines.Add('Bundle profile: InitialInstall - excludes .git, **/.venv and **/.venv-docx (any depth), .githooks, .github, .pm-ai-cache/network-source, .cursor, .vscode, code/VBA, code/参照用, code_java build/cache dirs, pm-ai-package-release/, output/, code/output/, code/python/output/, **/plan, **/plans, **/__pycache__, **/.pytest_cache, build_cache, root xlwings install bat, code workspace file (see package_workspace_copy.ps1).')
