...
function Ensure-PythonEmbedCache {
    ...
    try {
        # PS 5.1: nested try/finally があると字句解析が壊れる環境があるため 1 段の try/finally にまとめる
...
function Build-PmAiDesktopLauncherBatContent {
    ...
    # javafx.web requires javafx.media (win.jar + add-modules). Module path joined below (no fragile PS 5.1 tokens in comments).
    $javaFxWinJarNames = @(
...
"@
    ...
    # AppPaths.DISPATCH_USAGE_GUIDE_DOCX (dispatch usage guide .docx at repo root; Initial + VersionUpgrade).
    $dispatchUsageDocxLeaf = (-join @(
            [char]0x914D, [char]0x53F0, [char]0x30B7, [char]0x30B9, [char]0x30C6, [char]0x30E0,
            [char]0x4F7F, [char]0x3044, [char]0x65B9, [char]0xFF08, [char]0x6574, [char]0x7406, [char]0x7248, [char]0xFF09
        )) + '.docx'
    $verifyDispatchDocx = Join-Path $data $dispatchUsageDocxLeaf
    if (-not (Test-Path -LiteralPath $verifyDispatchDocx)) {
        $srcDispatchDocx = Join-Path $WorkspaceRootPath $dispatchUsageDocxLeaf
        throw @"
Bundle incomplete: missing dispatch usage guide Word file.
Expected in bundle: $verifyDispatchDocx
Place at repo root: $srcDispatchDocx
(AppPaths.DISPATCH_USAGE_GUIDE_DOCX; excluded manual/.venv-docx/ is unrelated.)
"@
    }
...
"@
    ...
    $rmLines.Add('Master *.txt under code/ and repo-root dispatch usage guide .docx are always copied (see package_app_mandatory_code_paths.txt; AppPaths.DISPATCH_USAGE_GUIDE_DOCX).')
    if ($BundleKind -eq 'InitialInstall') {
        $rmLines.Add('Bundle profile: InitialInstall - excludes .git, **/.venv and **/.venv-docx (any depth), .githooks, .github, .pm-ai-cache/network-source, .cursor, .vscode, code/VBA, 国分工場_実行環境/ (local unpacked install mirror), code/参照用, code_java build/cache dirs (incl. package_input_rdp/), pm-ai-package-release/, rpa_luncher_release/, tools/, preview_cache/, log/, output/, code/output/, code/python/output/, **/plan, **/plans, **/__pycache__, **/.pytest_cache, build_cache, root xlwings install bat, code workspace file (see package_workspace_copy.ps1).')
    ...
    if ($BundleKind -eq 'InitialInstall' -or $BundleKind -eq 'VersionUpgrade') {
        ...
        # exclude_rules.json for env tab bootstrap (MainShellController.maybeFillEmptyBootstrap)
...
if (Test-Path -LiteralPath $VersionTxtPath) {
    ...
    if (-not [string]::IsNullOrWhiteSpace($rawTxt)) {
        $nlParts = [regex]::Split($rawTxt, "`r?`n")
        if ($null -ne $nlParts -and $nlParts.Length -gt 0) {
            $firstLine = [string]$nlParts[0].Trim()
        }
    }
...
# Portable self-update baseline: version.txt next to PMD.exe (not inside pm-ai-data in ZIP).
foreach ($bd in @($bundleOutInitial, $bundleOutUpgrade)) {
    if (Test-Path -LiteralPath $VersionTxtPath) {
