...
todos:
  ...
  - id: decision-8-dual-running
    ...
    status: pending
  - id: bootstrap-toolchain-decisions
    content: L0 ワークストリーム（§L0〜L8 の L0）：Temurin/JavaFX 21 LTS と Gradle 候補の扱い・code_java サブモジュール案・依存固定を decision-l0／bootstrap-javafx と一本化
    status: pending
  - id: python-bridge-protocol
    content: L1：終了コード一覧・cancel.flag・NDJSON type 一覧を ipc-line.schema／decision-1 と整合（COMPARE_* フラグ対応表）
    status: pending
  - id: column-manifest-export
    content: L2：planning_core.export_schema と columns.json の CI ゲートを decision-2／sheet-ui-parity に接続
    status: pending
  - id: workspace-settings-migration
    content: L3：%APPDATA% と .workspace の settings 規約を決定値 §2 の単一 settings.json 案とマッピング
    status: pending
  - id: etl-python-parity-impl
    content: L4：select_latest_files／fetch_latest_files 統一・tests/golden・PQ-B Upsert 保留の解消を decision-3／powerquery-etl-parity で追跡
    status: pending
  - id: screens-ia-wireframes
    content: L5：暫定 8 画面と決定値 4 画面のワイヤー収斂・状態モデル検証（sheet-ui-parity）
    status: pending
  - id: dist-jpackage-setup
    content: L6：ZIP/run.bat 確定値と jpackage MSI 候補・embedded Python パスの優先順位を decision-5 で確定
    status: pending
  - id: qa-ci-oss-audit
    content: L7：TestFX 採否・CI 先・oss-perpetual-free-audit.csv と oss_licenses.md の役割分担を decision-6／7 で確定
    status: pending
  - id: dual-running-policy
    content: L8：プラン同期の手作業 vs pre-commit・.lock 排他・xlsm フォールバックを decision-7／8 と運用手順に落とす
    status: pending
isProject: false
---
...
4. **Python ETL 実装**を着手する際は、**§実装前に固める取り決め（L0〜L8）の L4** および **決定値 §3**・**`fetch_latest_files`**（実装名 **`select_latest_files` 案との統一**は L4 参照）方針、`powerquery-etl-parity`／`pq-m-export-from-workbook` の TODO と整合させる。
5. **UNC からの最新ファイル選定**を共通化する API（**`planning_core.etl.select_latest_files`** 等）は **L4** を正とし、実装・リネーム時に本プランと [plan/power-query-m-sources.md](power-query-m-sources.md) の索引を更新する。
...
以下と **§実装前に固める取り決め（決定値）**・**§実装前に固める取り決め（L0〜L8）** を往復参照する。**確定方針は決定値節を正**とし、**論点別のラベル**は **L0〜L8 節**を参照する。

**ギャップ項目と取り決め（L0〜L8 節のラベル）の対応**: **1** xlwings → **L1**（IPC）。**2** マクロブック分解 → **L2**（列・契約）／**L3**（Workspace）／**L8**（二重運用）。**3** インストール → **L0**（ツールチェーン）／**L6**（配布）。**4** 複数 PQ → **L4**（PQ 代替 ETL・`select_latest_files`／`fetch_latest_files`）。**5** 名前定義 PQ → **L2**／**L3**／**L4**。**6** 選択的 Java 化 → **L7**（QA・CI）・本文フェーズ 6。

※ **決定値節の「L0〜L8 ナビ」**は **### 1〜8 の見出し番号**と対応しており、**L0〜L8 節の L4（PQ）＝決定値の ### 3** など **番号がずれる**。混同時は **§実装前に固める取り決め（L0〜L8）** 冒頭の対応説明を参照する。
...
1. **xlwings 依存**  
   ...
   **移行後の目標**: **A のみ**とし、**ロジックが Excel ランタイムに依存しない**ことを検証する。**B は本プロジェクトの「Excel 論理切り離し」方針により採用しない**。実務的には **A を優先**し、どうしても必要な箇所だけ C を検討するのが安全。（**対応: L1**）

2. **マクロブックの役割の分解**  
   現状は「UI（シート）」「設定（設定_環境変数 等）」「Python へのパス受け渡し」が同一 `.xlsm` に混在。JavaFX 化では **「プロジェクトディレクトリ＋アクティブなタスクブックパス」**のような概念をアプリ側で明示管理する必要がある。（**対応: L2／L3／L8**）

3. **インストール単位**  
   Python ランタイム・依存パッケージ・（必要なら）Excel／xlwings の bundling 方針（インストーラ、社内配布用 zip、バージョン固定）を早期に決める。（**対応: L0／L6**）

4. **複数 Power Query パイプライン**  
   上記のとおり **UNC・「最新」指標・変換内容がクエリごとに異なる**。さらに **同一 UNC でも**「最新 1 ファイルだけ」と「新しい順に N 本を縦結合」など **ファイル集合のポリシーが異なる**。JavaFX 化では **ETL ジョブをクエリ単位（またはドメイン単位）にモジュール化**し、共有の「フォルダからファイル列を得る」ユーティリティを **日付列の意味（created / accessed / modified）・先頭 N 件・非表示除外**までパラメータ化すると再発バグを減らせる。（**対応: L4**）

5. **Excel 名前定義に依存する PQ**  
   `_q結果_配台表` のように **`Excel.CurrentWorkbook()` で名前を引く**クエリは、JavaFX では **名前定義を廃し**、**出力ルートパスをアプリ設定で明示**する。読み取りは **PQ なしでファイル API 十分**なことが多い。（**対応: L2／L3／L4**）

6. **選択的 Java 化の境界**  
   UI を JavaFX にした後も **計算の大半は Python のまま**が前提。**Java に移す候補**は、(a) **プロファイルで全体時間の有意な割合**を占める処理、(b) **入力出力がファイル／行列として明確**でゴールデンテストしやすい処理、(c) **業務ルールの変更頻度が低い**純粋計算、などに限定する。**Gemini 連携・環境依存が強い部分**は Python 継続が現実的なことが多い。（**対応: L7**・フェーズ 6）
...
**L0〜L8 ナビ（本節＝決定値／### 1〜8 との対応）**: **L0**＝直下「### L0. 基盤・ツールチェーン」。**L1**＝「### 1. Python ↔ …」。**L2**＝「### 2. データコントラクト…」。**L3**＝「### 3. PQ 代替 ETL…」。**L4**＝「### 4. 画面…」。**L5**＝「### 5. 配布…」。**L6**＝「### 6. 品質保証…」。**L7**＝「### 7. ドキュメント…」。**L8**＝「### 8. 失われやすい運用…」。§「Excel との論理的切り離し」は **L0・L8 と整合する補助節**（業務ロジックと成果物形式の境界）。**別節「§実装前に固める取り決め（L0〜L8）」**では **論点別ラベル**（例: **新L4**＝PQ＝本節の **### 3**）を用いるため、**番号がずれる**（§主要な技術的ギャップの注記参照）。
...
本節は **実装着手前に論点を三分類（決定／候補（要確認）／保留）**して整理した **一覧骨子**である。**詳細な確定値・IPC フィールド・表形式の契約**は **§実装前に固める取り決め（決定値）** を正とする。§「決定値」の **### 1〜8 の番号**と本節の **L1〜L8** は **対応するが同一ではない**（例: 本節 **L4**＝PQ 代替 ETL は決定値の **### 3**、本節 **L5**＝画面は **### 4**）。重複記述は避け、下記から **決定値・YAML の `decision-*`** へリンクする。

### L0. 基盤・ツールチェーン

- **候補（要確認）**: JDK＝Temurin **21** LTS、JavaFX＝**21** LTS、ビルド＝**Gradle（Kotlin DSL）**。※決定値 §L0 は **Maven** を確定しているため、Gradle へ変える場合は **`decision-l0-toolchain-finalize` で改めて確定**すること。
- **候補（要確認）**: `code_java/` サブディレクトリ構成として **`app/`**（JavaFX 起動・画面）、**`core/`**（業務モデル）、**`bridge/`**（Python 連携）、**`etl/`**（PQ 代替）、**`charts/`**、**`distribution/`**。
- **決定**: OS／ロケール・JVM 既定は **§実装前に固める取り決め（決定値）の ### L0** に従う（**Windows 11 限定**、`-Dfile.encoding=UTF-8`、`Locale.JAPAN`、`Asia/Tokyo`）。※「Windows 10/11」のように幅を持たせる必要が出た場合は **保留→決定値・Bootstrap の更新タスク**で扱う。
- **保留**: ControlsFX／POI／JFreeChart 等の **バージョン固定**は **`bootstrap-javafx` 着手時**に確定し、決定値 §L0 に追記する。

→ 詳細: §実装前に固める取り決め（決定値）の **### L0**・YAML `bootstrap-javafx`／`decision-l0-toolchain-finalize`・TODO **`bootstrap-toolchain-decisions`**

### L1. Python ↔ JavaFX IPC 契約

- **決定（方針）**: 起動は **`ProcessBuilder`**。子プロセスに **`TASK_INPUT_WORKBOOK`**、**`PYTHONUTF8=1`**、**`PYTHONIOENCODING=utf-8`** を付与する（決定値 §1 と整合）。
- **決定（方針）**: **stdout に NDJSON**（`type`: `progress`｜`log`｜`error`｜`done` 等）。既存の **`log/stage_vba_exitcode.txt`** 等は **VBA 互換のフォールバック**として維持する。
- **候補（要確認）**: 終了コード **`0`**＝成功、**`1`**＝一般エラー、**`2`**＝入力不正、**`3`**＝`PlanningValidationError`（既存）、**`4`**＝キャンセル、**`>=10`**＝未分類。決定値 §1 では **`2`**＝致命的／**`9`**＝ユーザー中断としており **番号の対応は未一本化（保留）**。`COMPARE_GANTT_ALLOW_PLAN_OVERLAP` 等の **環境フラグとの対応表**は IPC 仕様書（`decision-1-python-bridge-spec`）で確定する。
- **決定（方針）**: キャンセルは **協調的**（例: **`log/cancel.flag`** ＋ Python 側の定期チェック）。**`Process.destroyForcibly`** は最後の手段。
- **決定（方針）**: 同一 Workspace 内の **段階1／段階2** は **直列**。参照系（ETL）は **並列可**（決定値の「UI 単一実行」と矛盾しない範囲で設計）。

→ 詳細: §実装前に固める取り決め（決定値）の **§1（Python ↔ JavaFX）**・`ipc-line.schema.json`・YAML `decision-1-python-bridge-spec`・TODO **`python-bridge-protocol`**

### L2. 列定義マニフェスト

- **決定**: **真の正**は Python **`_core.py` の `PLAN_*`／`TASK_*` 定数**（[code/python/planning_core/_core.py](code/python/planning_core/_core.py)、数十件）。
- **候補（要確認）**: 生成物として **`code/python/planning_core/schema/columns.json`** を **`python -m planning_core.export_schema`**（等）で自動エクスポート。
- **決定（方針）**: JavaFX は起動時に JSON をロードし **SpreadsheetView の列定義を駆動**。CI で **定数追加→JSON 未更新**を検知する。

→ 詳細: §実装前に固める取り決め（決定値）の **§2（データコントラクト）**・`columns.schema.json`・YAML `decision-2-data-contract`・TODO **`column-manifest-export`**

### L3. 設定と Workspace 概念

- **候補（要確認）**: ユーザー設定 **`%APPDATA%\工程管理AI\settings.json`** ＋ プロジェクト設定 **`<workspace>/.workspace/settings.json`**。決定値 §2 は **`settings.json` をアプリ実行ディレクトリ直下**としており、**パス規約の一本化は保留**（マイグレーション設計で確定）。
- **決定（方針）**: 旧「**設定_環境変数**」シートの **キー名は同名で保持**。**`TASK_INPUT_WORKBOOK`** は Workspace から自動算出し **Python に渡す**（運用方針として **OS の恒久環境変数へ書き込まない**ことを目標とする）。
- **決定（方針）**: **Workspace**＝旧マクロブックの所在フォルダを抽象化した概念。**アクティブ Workspace の切替**は UI で行う。

→ 詳細: §実装前に固める取り決め（決定値）の **§2**・YAML `decision-2-data-contract`・TODO **`workspace-settings-migration`**

### L4. PQ 代替 ETL（PQ-A〜D の Python 同等）

- **決定（方針）**: **Python ETL を主経路**（意思決定表の推奨を確定）。
- **候補（要確認）**: 共通ユーティリティ **`planning_core.etl.select_latest_files(unc, date_kind: created/accessed/modified, top_n, exclude_hidden=True)`**（命名は実装時に既存の **`fetch_latest_files`** 案と **統一**する）。
- **決定（方針）**: 出力は既存 **「加工計画DATA」系シートと同型の xlsx**（互換維持）。**PQ-D** は **xlsx 直読**。
- **決定（方針）**: ゴールデンは **`tests/golden/pq_*`**（または **`tests/fixtures/`**）に匿名化サンプルを置き **pandas `assert_frame_equal`** で比較。
- **保留**: PQ-B のコメント「Upsert」と実 **`in`（ソートのみ）** の差異 → **業務確認後**に仕様確定（§PQ-B・決定値 §3 参照）。

→ 詳細: §実装前に固める取り決め（決定値）の **§3（PQ 代替 ETL）**・§Power Query 正本・YAML `decision-3-etl-python-parity`・TODO **`etl-python-parity-impl`**

### L5. 画面 IA・主要画面・状態モデル

- **候補（要確認）**: トップ IA＝**左サイドバー**（プロジェクト／画面ナビ）＋ **メインのタブ領域**。
- **候補（要確認）**: 主要画面（暫定）＝**ホーム／加工計画DATA／配台計画_タスク入力／実績明細／結果_設備ガント／実績設備ガント／配台試行順パターン別／設定**（決定値の「4 主要画面」と **粒度が異なる→ワイヤーで収斂**）。
- **決定（方針）**: 状態モデル **`Idle`／`Loading`／`Editing(dirty)`／`Running`／`Done`／`Error`**。
- **決定（方針）**: **矩形貼り付け・Tab／Enter 移動・IME** は POC で確認し、不足は **`sheet-ui-parity`** の評価項目に追加。

→ 詳細: §実装前に固める取り決め（決定値）の **§4（画面）**・YAML `decision-4-screens-ia`・TODO **`screens-ia-wireframes`**

### L6. 配布・運用・セキュリティ

- **候補（要確認）**: **`jpackage`** による **MSI**（社内配布）。**コード署名**は当面なし。
- **候補（要確認）**: **`app/runtime/python/`** に **embedded Python** ＋ 初回起動で **`pip install`**（既存 **`setup_environment.py`** を再利用）。決定値 §5 は **ZIP＋`run.bat`** 確定のため **パッケージ形式は保留**。
- **候補（要確認）**: 更新は当面 **手動 zip／MSI 上書き**。**SemVer** タグ。
- **決定（方針）**: Gemini 資格情報は **`%APPDATA%\工程管理AI\gemini.cred`**（既存 **`encrypt_gemini_credentials.py`** の暗号化を流用）を **候補パス**として設計し、決定値 §5 の「環境変数／`.env`」方針と **整合させる**。
- **決定（方針）**: **UNC** は到達不能・遅延時の UI（**再試行・スキップ**）を必須化。決定値の「即時エラー無リトライ」との **緩急は実装時に統一**。

→ 詳細: §実装前に固める取り決め（決定値）の **§5（配布）**・YAML `decision-5-distribution`・TODO **`dist-jpackage-setup`**

### L7. テスト・CI・OSS 監査

- **候補（要確認）**: UI は **TestFX**（Apache 2.0）でスモーク。決定値 §6 は **手動 QA・TestFX 不採用**→ **採否は保留**（OSS 台帳・法務とセットで確定）。
- **決定（方針）**: ETL は **pytest＋pandas** で **完全一致**（dtype／列順含む）。数値計算列のみ **許容誤差**を別表で定義。
- **決定（方針）**: 配台コアは **既存 Python テスト**を流用。
- **候補（要確認）**: CI は **GitHub Actions** または **Jenkins**。**Java と Python はジョブ分離**。
- **決定（方針）**: OSS 監査台帳 **`Production-Control-System/plan/oss-perpetual-free-audit.csv`**（列: name, version, license, perpetual_free, evidence_url, decision）。※既存 TODO は **`oss-perpetual-free-audit`**／**`plan/oss_licenses.md`** との **関係を整理**して二重記載を避ける。

→ 詳細: §実装前に固める取り決め（決定値）の **§6（品質保証）**・YAML `decision-6-qa-test-bat`・TODO **`qa-ci-oss-audit`**

### L8. ドキュメント運用と二重運用ポリシー

- **候補（要確認）**: プラン同期: **`plan/...plan.md` を正**とし、**`.cursor/plans/`** へ **pre-commit フック**で自動コピー（[.cursor/hooks](.cursor/hooks)）。決定値 §7 は **手作業同期**を確定しているため **運用は保留で一本化**。
- **決定（方針）**: 配台ルールの **既定の正**は **Python**。Java 化したモジュールのみ **Java を正**にスイッチ（フェーズ 6）。
- **候補（要確認）**: 二重運用期間、同一 Workspace を JavaFX が **`.lock` で排他**、Excel 編集時はアラート。
- **候補（要確認）**: Excel 版 **`.xlsm` は廃止せず**、**フィーチャーフラグ**でフォールバック可能にする。

→ 詳細: §実装前に固める取り決め（決定値）の **§7（ドキュメント）**・**§8（運用）**・YAML `decision-7-docs-licenses`／`decision-8-dual-running`・TODO **`dual-running-policy`**

---

## 意思決定が必要な論点（実装前に固めると安全）

> **更新**: L0 と 1〜8 の決定値は **§ 実装前に固める取り決め（決定値）** に確定済み。**論点と L0〜L8 の対応**は **§実装前に固める取り決め（L0〜L8）** を主とする。下表は **要約の歴史記録**として残す。

確定した運用・技術の値は、本文の **§実装前に固める取り決め（決定値）**・**§実装前に固める取り決め（L0〜L8）** および YAML フロントマターの **`decision-l0-toolchain-finalize`** ～ **`decision-8-dual-running`**、`bootstrap-toolchain-decisions` 等（下記 TODO）をワーク分解の起点とする。

| 論点 | 選択肢のイメージ | L（§実装前に固める取り決め（L0〜L8）） |
|------|------------------|----------------------------------------|
| 計算コア | **既定: Python 継続**。重い部分のみ **計測後に Java 部分移植（ハイブリッド）**。**全面 Java 移植はスコープ外**として扱う | **L0**／**L7**／フェーズ6 |
| **Java ソースのルート** | **`code_java/`** に統一。**`code/` は既存 Excel／Python ラインのまま**とし衝突を避ける | **L0** |
| **OSS の永続無料** | **必須**。有償サブスク・オープンコアの実質有償・二重ライセンスで有償が必要な解釈は原則採用しない。**依存台帳（`oss-perpetual-free-audit`）**で証跡を残す | **L0**／**L7** |
| **グリッド UI（Excel 親和・OSS）** | **既定: Java と親和性の高い OSS を優先**（**ControlsFX SpreadsheetView** を POC 起点）。単純一覧は **TableView** で可。**Apache POI** で書式・往復検証を検討。**永続無料**・ライセンス・保守で最終確定 | **L0**／**L2**／**L5** |
| **チャート／グラフ** | **既定: JFreeChart を優先**。軽量・シンプルな系列は **JavaFX Chart** を **状況に応じ選択**。埋め込み（SwingNode／画像）・LGPL 可否は POC・法務で確定 | **L0**／**L5** |
| 保存形式 | 当面 **xlsm 互換** vs 早めに **DB/JSON＋エクスポート** | **L2**／**L3** |
| xlwings 代替 | **Python をファイル中心に寄せる（確定）**。Excel 常駐（xlwings）は **不採用**（§「Excel との論理的切り離し」） | **L1**／**L3** |
| **加工計画DATA 供給** | **Python ETL で PQ 同等（推奨して検証）** vs バッチ vs Excel 自動化のみ | **L4** |
| **加工計画DATA_実績比較用 供給** | 同上（**N ファイル縦結合・複数 `抽出時間`**。単一ファイル版との **共通化と差分**をコードで分離） | **L4** |
| **加工実績明細（_q）供給** | 同上（**累積・完了率・複数日付キー**で検証コストが一段高い） | **L4** |
| **結果_配台表の参照** | **出力 xlsx 直読＋設定で出力パス**（名前「フォルダパス」を廃止）。列型は M の `TransformColumnTypes` と整合 | **L2**／**L3**／**L4** |
