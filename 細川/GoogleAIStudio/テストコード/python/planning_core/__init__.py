...
【外部から直接使う入口（他 .py から import される想定）】
    ...
    - ``refresh_dispatch_trial_pattern_list_sheet_only()`` … P1～P4 の決定論パターンを既定で一覧化
      （P3 は ``DISPATCH_TRIAL_PATTERN_P3_BUFFER_DAYS``）。ランダム（R*）は ``DISPATCH_TRIAL_PATTERN_RANDOM_COUNT``>0 のときのみ（既定 0）。
