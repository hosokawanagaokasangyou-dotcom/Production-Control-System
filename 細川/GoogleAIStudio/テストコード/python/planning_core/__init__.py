...
【外部から直接使う入口（他 .py から import される想定）】
    ...
    - ``refresh_dispatch_trial_pattern_list_sheet_only()`` … 納期優先・機械名+納期・ランダム等の
      配台試行順パターンを一覧化したシートをマクロブックに作成（`apply_dispatch_trial_pattern_list_sheet.py` / xlwings）。
      ランダム件数等は ``DISPATCH_TRIAL_PATTERN_*`` 環境変数または「設定_環境変数」シート（雛形 TSV 参照）。
