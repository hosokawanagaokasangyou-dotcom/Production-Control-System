...
【外部から直接使う入口（他 .py から import される想定）】
    ...
    - ``refresh_dispatch_trial_pattern_list_sheet_only()`` … P1～P3 の決定論パターンを既定で一覧化
      （P3 は機械グループ単位で原反投入日〜納期の日数合計が短い順）。ランダム（R*）は ``DISPATCH_TRIAL_PATTERN_RANDOM_COUNT``>0 のときのみ（既定 0）。
