...
- 本モジュールは **マクロブック（.xlsm）と同じフォルダ直下の** ``python/`` **配下**
  ...
  ``RunPython`` が参照する Python の ``sys.path`` には、この ``python`` が自動では入らないことが多いため、
  **マクロブックと同じフォルダに ``xlwings.conf.json`` を置き、PYTHONPATH に上記 ``python`` フォルダを
  含めることが実質必須**（これが無いと ``import xlwings_console_runner`` や ``planning_core`` 等が解決できない）。
  リポジトリ同梱の ``xlwings.conf.json`` を参照。
