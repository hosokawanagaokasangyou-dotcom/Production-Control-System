...
【ソース上の構成（=#= 見出しでスクロール検索可能）】
    ...
    - 配台不要（分割自動・設定シート・xlwings / VBA 用 TSV フォールバック）
...
【依存】
    pandas, openpyxl, xlwings（起動中の Excel への反映・保存）, google.genai, cryptography（暗号化認証 JSON 時）
    API キーはマクロブック「設定」B1 の JSON パスからのみ読む（環境変数 GEMINI_API_KEY は使わない）。JSON は平文または format_version 2（Fernet+PBKDF2）。
    暗号化 JSON の復号は ``planning_core`` 内の定数のみ使用（値はソースにあり、社内共有資料やログには書かないこと）。

【初回環境（Windows / Excel あり）】
    - Python 3.10+ 推奨。初回は ``py -3 setup_environment.py`` またはマクロ「環境構築」（requirements.txt を一括導入）
    - Excel の「ファイル → オプション → セキュリティ センター → マクロの設定」で
      VBA プロジェクト オブジェクト モデルへのアクセスを必要に応じて許可（企業ポリシーに従う）
    - xlwings はデスクトップ版 Excel を自動操作する。ブックはローカルパス推奨（クラウド同期上のロックは避ける）
"""
...
# =========================================================
# Gemini API キー: マクロブック TASK_INPUT_WORKBOOK の「設定」B1 に JSON のフルパス（またはブック相対パス）
# 平文 JSON または暗号化 JSON（encrypt_gemini_credentials.py）。暗号化の復号は常に _GEMINI_CREDENTIALS_PASSPHRASE_FIXED のみ。
...
# マクロブック「設定」B1: 社内共有上の Gemini 認証 JSON へのパス（TASK_INPUT_WORKBOOK 確定後に API_KEY を解決）
...
# マクロブック「設定_配台不要工程」: A〜E は openpyxl 保存を優先。ロック時は xlwings で同期保存、それも不可なら TSV→VBA 反映。
...
def analyze_global_priority_override_comment(
    ...
    if not API_KEY:
        logging.info("Gemini API キー未設定のためメイン再優先特記の AI 解析をスキップしました。")
        if ai_sheet_sink is not None:
...
def _write_main_sheet_gemini_usage_via_xlwings(
    ...
    """ブックが Excel で開いているとき、メインシート P 列 16 行目以降にサマリを書き save する。"""
    xw = _xlwings_try_import()
    if xw is None:
        return False
    book = _xlwings_find_book_by_path(os.path.abspath(macro_wb_path))
    if book is None:
        return False
    start_r, col_p, clear_n = 16, 16, 120
    ws_main = None
    ...
    if ws_main is None:
        ...
        return False
    lines = text.split("\n") if (text or "").strip() else []
    n_lines = min(len(lines), clear_n)
    last_clear = start_r + clear_n - 1
    try:
        if last_clear >= start_r:
            ws_main.range((start_r, col_p), (last_clear, col_p)).clear_contents()
        if n_lines:
            col_vals = [[lines[i]] for i in range(n_lines)]
            ws_main.range((start_r, col_p), (start_r + n_lines - 1, col_p)).value = (
                col_vals
            )
        book.save()
        logging.info(
            "%s: メインシート P%d 以降に AI 利用サマリを xlwings で保存しました。",
            log_prefix,
            start_r,
        )
        return True
...
def write_main_sheet_gemini_usage_summary(wb_path: str, log_prefix: str) -> None:
    """Gemini 利用サマリを log に書き、可能なら openpyxl でメイン P 列へ保存。不可なら xlwings。開いていない場合は VBA 用テキストのみ。"""
    ...
    if wb_path and os.path.isfile(wb_path):
        ...
        if not disk_ok:
            try:
                disk_ok = _write_main_sheet_gemini_usage_via_xlwings(
                    wb_path, text, log_prefix
                )
            except Exception as ex:
                logging.warning(
    ...
    if text.strip():
        logging.info(
            "%s: メイン P 列は openpyxl / xlwings いずれでも保存できませんでした。"
...
def analyze_task_special_remarks(tasks_df, reference_year=None, ai_sheet_sink: dict | None = None):
    ...
    if not API_KEY:
        logging.info("Gemini API キー未設定のためタスク特別指定のAI解析をスキップしました。")
        if ai_sheet_sink is not None:
...
def _xlwings_apply_planning_sheet_conflict_styles(
    wb_path: str, sheet_name: str, num_data_rows: int, conflicts_by_row: dict
) -> bool:
    """ブックが Excel で開いているとき、矛盾ハイライトを xlwings で適用して save する。"""
    xw = _xlwings_try_import()
    if xw is None:
        return False
    book = _xlwings_find_book_by_path(os.path.abspath(wb_path))
    if book is None:
        return False
    try:
    ...
    hdr = ws.range((1, 1), (1, last_col)).value
    if hdr is None:
        return False
    hdr_row = hdr if isinstance(hdr, list) else [hdr]
    header_map: dict[str, int] = {}
    for idx, v in enumerate(hdr_row):
        if v is not None:
            header_map[str(v).strip()] = idx + 1
    last_row = max(2, 1 + int(num_data_rows))
    yellow_rgb = (255, 242, 204)
    red_rgb = (255, 0, 0)
...
def apply_planning_sheet_conflict_styles(wb_path, sheet_name, num_data_rows, conflicts_by_row):
    ...
    try:
        ...
        try:
            ...
            if _xlwings_apply_planning_sheet_conflict_styles(
                wb_path, sheet_name, num_data_rows, conflicts_by_row
            ):
                _remove_planning_conflict_sidecar_safe()
                if conflicts_by_row:
                    logging.info(
                        f"特別指定_備考と列の矛盾: {len(conflicts_by_row)} 行を '{sheet_name}' でハイライトしました（xlwings）。"
                    )
            else:
...
#       保存ロック時は xlwings で A:E のみ同期するフォールバックあり
...
def _log_exclude_rules_sheet_debug(
    ...
    OPENPYXL_RETRY_WAIT, OPENPYXL_VBA_FALLBACK, MATRIX_TSV_WRITTEN,
    XLWINGS_ATTACH_OPEN_FAIL, XLWINGS_SAVE_OK, XLWINGS_SAVE_FAIL, XLWINGS_UNAVAILABLE, XLWINGS_BOOK_NOT_OPEN,
    ...
    if event in (
        ...
        "XLWINGS_ATTACH_OPEN_FAIL",
        "XLWINGS_SAVE_FAIL",
        "XLWINGS_UNAVAILABLE",
        "XLWINGS_BOOK_NOT_OPEN",
        "E_SIDECAR_WRITTEN",
        "OPENPYXL_SAVE_FAIL",
...
def _paths_equivalent_for_excel_workbook(disk_path: str, workbook_full_name: str) -> bool:
    """ディスクパスと Excel が返す FullName が同一ブックか（表記ゆれを多少吸収）。"""
    fn = str(workbook_full_name).strip()
...
def _xlwings_find_book_by_path(abs_disk_path: str):
    """起動中の Excel インスタンスから、パスが一致する xlwings Book を返す。無ければ None。"""
    xw = _xlwings_try_import()
    if xw is None:
        return None
    target = os.path.abspath(abs_disk_path)
    try:
        apps_iter = xw.apps
    except Exception:
        return None
        ...
        try:
            for book in app.books:
                try:
                    ...
                    continue
                if _paths_equivalent_for_excel_workbook(target, fn):
                    return book
        except Exception:
            continue
    return None
...
def _xlwings_release_book_after_mutation(book, info: dict, mutation_ok: bool) -> None:
    """mode が quit_excel のときは保存せず閉じて Excel を終了。keep で当セッションで開いたブックは失敗時のみ閉じる。"""
    if book is None:
        return
...
def _xlwings_attach_open_macro_workbook(
    macro_wb_path: str, log_prefix: str
) -> tuple[object, dict] | None:
    """
    マクロブックを xlwings で取得する。
    ...
    if xw is None:
        _log_exclude_rules_sheet_debug(
            "XLWINGS_UNAVAILABLE",
            log_prefix,
            "xlwings がインポートできません（pip install xlwings）。",
            details=f"path={macro_wb_path}",
        )
        return None

    abs_path = os.path.abspath(macro_wb_path)
    book = _xlwings_find_book_by_path(abs_path)
    if book is not None:
    ...
    if n_apps > 0:
        for app in xw.apps:
            try:
                ...
                pass
            try:
                b = app.books.open(abs_path)
                return b, {"mode": "keep", "opened_wb_here": True}
            except Exception:
                continue
    ...
    try:
        ...
        app.display_alerts = False
        b = app.books.open(abs_path)
        return b, {"mode": "quit_excel", "opened_wb_here": True}
    except Exception as ex:
        _log_exclude_rules_sheet_debug(
            "XLWINGS_ATTACH_OPEN_FAIL",
            log_prefix,
...
def _xlwings_attach_workbook_for_tests(
    ...
    *,
    ...
    """
    テスト用: 既に開いているブックにだけ接続するか、allow_dispatch_open で表示付き新規 App を立ち上げて開く。
    戻り値: (Book, release_info, attach_label) / 失敗時 None。
    """
    xw = _xlwings_try_import()
    if xw is None:
        print("xlwings が未インストールです: pip install xlwings", file=sys.stderr)
        return None
    abs_path = os.path.abspath(book_path)
    book = _xlwings_find_book_by_path(abs_path)
    if book is not None:
        return book, {"mode": "keep", "opened_wb_here": False}, "xlwings: 起動中のブック"
    ...
    try:
        ...
        app.display_alerts = False
        b = app.books.open(abs_path)
        return b, {"mode": "quit_excel", "opened_wb_here": True}, "xlwings: 新規 App(表示) + Open"
    except Exception as ex:
        print(f"xlwings でブックを開けませんでした: {ex}", file=sys.stderr)
        return None


def _xlwings_sync_exclude_rules_sheet_and_save(
    wb_path: str, opy_ws, log_prefix: str
) -> bool:
    """
    openpyxl で編集済みの「設定_配台不要工程」を、Excel 上で開いている同一ブックへ A:E で反映して保存する。
    """
    xw = _xlwings_try_import()
    if xw is None:
        _log_exclude_rules_sheet_debug(
            "XLWINGS_UNAVAILABLE",
            log_prefix,
            "xlwings 未導入のため Excel 経由の保存をスキップしました。",
    ...
    if book is None:
        _log_exclude_rules_sheet_debug(
            "XLWINGS_BOOK_NOT_OPEN",
            log_prefix,
            "openpyxl 保存不可だが、該当ブックが Excel で開かれていません（xlwings で同期できず）。",
            details=f"path={abs_path}",
        )
        return False
    try:
        sht = book.sheets[EXCLUDE_RULES_SHEET_NAME]
    except Exception as ex:
        _log_exclude_rules_sheet_debug(
            "XLWINGS_SAVE_FAIL",
            log_prefix,
            f"xlwings でシート '{EXCLUDE_RULES_SHEET_NAME}' を取得できませんでした。",
            details=f"path={abs_path}",
            exc=ex,
        )
        return False
    max_r = max(1, int(opy_ws.max_row or 1))
    data: list[list] = []
    for r in range(1, max_r + 1):
        row: list = []
        for c in range(1, 6):
            row.append(opy_ws.cell(row=r, column=c).value)
        data.append(row)
    try:
        sht.range((1, 1), (max_r, 5)).value = data
        book.save()
    except Exception as ex:
        _log_exclude_rules_sheet_debug(
            "XLWINGS_SAVE_FAIL",
            log_prefix,
            "xlwings で設定シート A:E の反映または Save に失敗しました。",
            details=f"path={abs_path} rows={max_r}",
            exc=ex,
        )
        return False
    ...
    _log_exclude_rules_sheet_debug(
        "XLWINGS_SAVE_OK",
        log_prefix,
        "xlwings で設定シート A〜E を Excel 上のブックに反映し保存しました。",
...
EXCLUDE_RULES_SHEET_SYNC_MAX_COL = 5
EXCLUDE_RULES_MATRIX_CLIP_MAX_COL = 5


def _persist_exclude_rules_workbook(_wb, wb_path: str, ws, log_prefix: str) -> bool:
    """
    設定シートのディスク反映。まず openpyxl で save。
    ロック等で失敗したら Excel で同一ブックが開いていれば xlwings で A:E を同期して save。
    それも不可なら log に行列 TSV を出し、VBA「設定_配台不要工程_AからE_TSVから反映」で反映する。
    ...
    logging.info(
        "%s: 設定_配台不要工程は openpyxl で保存します（不可時は xlwings 同期、それも不可なら VBA 用行列 TSV）。",
        log_prefix,
    )
    ...
    if _xlwings_sync_exclude_rules_sheet_and_save(wb_path, ws, log_prefix):
        return True
    ...
    _log_exclude_rules_sheet_debug(
        "OPENPYXL_VBA_FALLBACK",
        log_prefix,
        "openpyxl / xlwings いずれでも保存できなかったため VBA 用行列 TSV を出力しました（ブックは Excel 上で手動反映が必要な場合があります）。",
        details=f"path={wb_path}",
    )
    return False
...
def load_attendance_and_analyze(members):
    ...
    if remarks_to_analyze:
        ...
        if cached_data is not None:
            ...
            logging.info("Gemini API キー未設定のため備考AI解析をスキップしました。")
        else:
