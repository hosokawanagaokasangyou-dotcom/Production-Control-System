...
' 全シートフォント統一で保護シートを一時解除するときのパスワード（空＝パスワード無しのみ。パスワード付きシートはここに設定）
...
' ・シート名「組み合わせ表」… skills シートの直後（Python は TEAM_ASSIGN_USE_MASTER_COMBO_SHEET で読込）
...
' 段階2 Python（planning_core._core）が読む任意シート。シート名はコード定数と一致させること。
Private Const SHEET_MACHINE_CHANGEOVER As String = "設定_依頼切替前後時間"
Private Const SHEET_MACHINE_DAILY_STARTUP As String = "設定_機械_日次始業準備"
...
' Ctrl+Shift+テンキー - → メインへ（Application.OnKey）。{109}=vbKeySubtract（テンキー -）。{SUBTRACT} は環境により OnKey が 1004 で失敗するため数値コードを使用
Private Const SHORTCUT_MAIN_SHEET_ONKEY As String = "^+{109}"

' メインシート（「メイン」「メイン_」「Main」、または名前に「メイン」を含む）を取得
' ※「〇月メインカレンダー」等を誤採用しないよう、名前に「カレンダー」を含むシートはフォールバック対象外。
' ※複数候補があるときはシート名が最短のものを優先（先頭一致のブックで誤った長い名前を避ける）。
Private Function MasterGetMainWorksheet(ByVal wb As Workbook) As Worksheet
    ...
    For Each ws In wb.Worksheets
        If InStr(ws.Name, "メイン") > 0 Then
            If InStr(ws.Name, "カレンダー") > 0 Then GoTo NextMainCand
            L = Len(ws.Name)
            If L < bestLen Then
                bestLen = L
                Set best = ws
...
' skills シート A 列 3 行目以降からメンバー名を収集し、重複除去・昇順ソートした配列（1..n）と件数 n を返す。skills が無い／0 件は n=0。
...
' 勤怠簿シート（メンバー名）へのローカル HYPERLINK 数式。シート名の単引用符は二重化、表示文字の " はエスケープ。
...
' メイン MAIN_ATTENDANCE_LINK_COL に勤怠簿リンクリストを書き直す。失敗しても呼び出し元の処理は継続。
' D 列 Clear でフォントが既定に戻るため、Clear 前に代表セルから名・サイズを退避し、書き込み後に復元する。
Private Sub MasterRefreshMainAttendanceLinkList(ByVal wb As Workbook)
    ...
    wsMain.Cells(1, MAIN_ATTENDANCE_LINK_COL).Value = "勤怠簿リンク"
    wsMain.Cells(1, MAIN_ATTENDANCE_LINK_COL).Font.Bold = True
    
    Call MasterCollectSortedSkillsMembers(wb, names, n)
    If n = 0 Then
        wsMain.Cells(MAIN_ATTENDANCE_LINK_FIRST_DATA_ROW, MAIN_ATTENDANCE_LINK_COL).Value = "（skills の A 列（3 行目～）にメンバーがありません）"
        GoTo FitCol
    End If
    ...
    For i = 1 To n
        ...
        Else
            wsMain.Cells(r, MAIN_ATTENDANCE_LINK_COL).Value = names(i) & "（勤怠シート未作成）"
        End If
        r = r + 1
    Next i
...
' メインシートの勤怠簿（メンバー別シート）への HYPERLINK 一覧を skills に基づき更新
Sub master_メインの勤怠リンクリストを更新()
    On Error GoTo EH
    MasterRefreshMainAttendanceLinkList ThisWorkbook
    MsgBox "メインシート（D 列）の勤怠簿リンクを更新しました。", vbInformation
    Exit Sub
EH:
    MsgBox "勤怠リンク更新エラー: " & Err.Description, vbCritical
End Sub

' -----------------------------------------------------------------------
' メンバー勤怠（出勤簿）シート: アクティブセル行の入力可能列 A～K を薄色でハイライト
' ※ ThisWorkbook モジュールに次を1つ追加してください（貼り付け後、イベントが有効になります）。
'
...
' ThisWorkbook.SheetSelectionChange から呼び出し（標準モジュールは Public 必須）
...
' メイン A15/B15・A12/B12 等: 結合セルでも左上セルの実値を取得（結合右隣が空扱いになるのを防ぐ）
...
' Excel セルの時刻（Double の日付シリアル・Date 型・文字列）から「時」0～23 を取得。解釈不能は -1。
' ※「時刻のみ」のセルは Value が数値になり IsDate(値) が False になり得るため、IsNumeric 経路が必須。
...
' メインの時刻セルを Date（時分）として解釈。解釈不能は False（時刻のみ Double 対応）。
...
' メイン A15・B15 = 定常開始・終了（カレンダー→出勤簿の「通常」出勤/退勤・機械カレンダーでは定常外スロットの A 列のみ着色）。空・不正時は False。
...
' - A列(5行目以降): メンバー名
' - 交点セル: 空白=通常 / *=年休 / 国=他拠点勤務 / 前休=午前年休・休憩時間1_終了～定常終了(午後休憩14:45～15:00。休憩1は通常行と同値12:00～12:50) / 後休=定常開始～休憩時間1_開始・午後年休（定常はメイン A15/B15、空なら 8:45～17:00）
...
Sub master_カレンダーからメンバー出勤簿を生成()
    ...
    If Not regFromMain Then
        MsgBox "定常時刻（メインシート A15/B15）が未設定です。メンバー別出勤簿を生成できません。", vbCritical
        GoTo CleanExit
    End If
    ...
    MsgBox "メンバー別出勤簿を生成しました。" & vbCrLf & _
           "カレンダーシート: " & CStr(calendarCount) & " / メンバー: " & CStr(memberCount) & " / レコード: " & CStr(totalRec) & vbCrLf & _
           "定常（通常・国・後休の出退・前休の終了）: " & Format$(TimeValue(regSt), "hh:nn") & " ～ " & Format$(TimeValue(regEn), "hh:nn") & _
           IIf(regFromMain, "（メイン A15/B15）", "（メイン未設定のため 8:45～17:00）"), vbInformation
...
Private Sub CollectAttendanceFromCalendar(ByVal wsCal As Worksheet, ByRef recMap As Object, ByRef totalRec As Long, ByVal regSt As Date, ByVal regEn As Date)
    ...
    Dim note As String
    ' 通常行と同じ休憩時間1（メンバー出勤簿の休憩時間1_開始/終了と一致。変更時は Case "" / 国 / Else と前休・後休を揃える）
    ...
    For r = 5 To lastRow
        ...
        For c = 2 To lastCol
            ...
            Else
                Select Case marker
                    ...
                    Case "国"
                        ...
                        note = "他拠点勤務"
                    Case "前休"
                        ' 午前は年休扱い。休憩時間1_終了～定常終了。午後休憩は通常どおり 14:45～15:00（昼食帯は出ず）
                        st = defB1e: en = regEn
                        b2s = TimeSerial(14, 45, 0): b2e = TimeSerial(15, 0, 0)
                        note = "前休"
                    Case "後休"
                        ' 定常開始～休憩時間1_開始 勤務。午後は年休扱い
                        st = regSt: en = defB1s
                        note = "後休"
                    Case Else
                        st = regSt: en = regEn
            ...
            ' rec(8) = 休暇区分、rec(9) = 備考（カレンダーからは入れない。再生成時は既存シート J～K を日付キーで復元・K は時刻/数値/文字いずれも可）
...
' メンバー出勤簿の退避用: セルにユーザー入力相当の値があるか（空・エラー値は False。数値の残業時間・文字列入力も True）
...
Private Sub FormatMemberAttendanceSheetLayout(ByVal ws As Worksheet, ByVal lastRow As Long)
    ...
    On Error GoTo 0
    ' 呼び出し元（勤怠一括生成など）が ScreenUpdating=False のときも True に戻さない
    On Error Resume Next
...
' 日本語環境では NumberFormatLocal = "General" が 1004 になることがあるため、標準表示は NumberFormat を使う
...
Private Sub WriteMemberAttendanceSheet(ByVal memberName As String, ByVal recs As Collection)
    ...
    Dim fmtR As Long
    Dim kv As Variant
    ' ws.Cells.Clear で書式も消えるため、再生成前のフォント（名・サイズ）を退避する
    Dim preserveFontName As String
    Dim preserveFontSize As Variant
    Dim samp As Range
    Dim lr0 As Long
    ...
    ' Clear 後は既定フォントに戻るため、再生成前に退避したフォントを A1:K の表範囲へ戻す
...
' =========================================================
' 機械カレンダー: 1 シート、skills と同型の 1～2 行見出し、A 列は日付時刻（1 時間単位のスロット）
' =========================================================
Public Sub master_機械カレンダーを作成()
    Dim wb As Workbook
    ...
    If wsSkill Is Nothing Then
        MsgBox "skills シートがありません。工程名・機械名の見出しをコピーできません。", vbExclamation, "機械カレンダー"
        Exit Sub
    End If
    
    Call MachineCalGetSkillsMode(wsSkill, lastC, useTwoHeader)
    If lastC < 2 Then
        MsgBox "skills シートに有効な列がありません。", vbExclamation, "機械カレンダー"
        Exit Sub
    End If
    
    Set wsOut = EnsureMachineCalendarSheet(wb, wsSkill)
    If wsOut Is Nothing Then
        MsgBox "「" & SHEET_MACHINE_CALENDAR & "」シートを用意できません。", vbExclamation, "機械カレンダー"
        Exit Sub
    End If
...
Sub master_アニメ付き_機械カレンダーを作成()
    Call AnimateButtonPush
    Call master_機械カレンダーを作成
End Sub
...
' 曜日略称（環境の表示言語に追随。日本語 Excel では 月火水木…）
...
Private Sub PreserveMachineCalendarGrid(ByVal ws As Worksheet, ByVal outMap As Object)
    ...
    hdr = Trim$(CStr(ws.Cells(1, 1).Value))
    On Error GoTo 0
    If StrComp(hdr, "日付", vbTextCompare) <> 0 And StrComp(hdr, "日付時刻", vbTextCompare) <> 0 Then Exit Sub
    
    lastC = ws.Cells(1, ws.Columns.Count).End(xlToLeft).Column
    lastR = ws.Cells(ws.Rows.Count, 1).End(xlUp).Row
    If lastR < 3 Then Exit Sub
    
    If StrComp(Trim$(CStr(ws.Cells(1, 2).Value)), "曜日", vbTextCompare) = 0 Then
        startEquipCol = 3
    Else
        startEquipCol = 2
    End If
...
' 3 行目以降を「カレンダー日」単位で薄い青のストライプ（同一日は同色）
...
' 定常開始～終了（メイン A15/B15）と重ならない 1 時間スロットの A 列（日時）のみ薄いオレンジで強調（B 列は日ストライプのまま）
Private Sub ApplyMachineCalendarOutsideRegularHoursTint(ByVal ws As Worksheet, ByVal wb As Workbook, ByVal lastRow As Long, ByVal lastCol As Long)
    ...
    For rr = 3 To lastRow
        ...
        ' 半開区間 [slotStart, slotEnd) と [regStart, regEnd) が重ならない → 定常外
...
' 出退時刻は本体でメイン A15/B15（定常）を読み、全カレンダー共通で適用（空なら 8:45～17:00）。
' =========================================================
...
Sub master_アニメ付き_依頼切替日次始業シートを更新()
    On Error GoTo EH
    Call AnimateButtonPush
    Call MasterEnsureMachineChangeoverTemplateSheets
    MsgBox "設定シートを更新しました。" & vbCrLf & _
           "・設定_依頼切替前後時間 … skills の工程+機械で未登録行を追補（準備/後始末の分は既存行はそのまま）" & vbCrLf & _
           "・設定_機械_日次始業準備 … skills の機械名で未登録行を追補（分は既存行はそのまま）", vbInformation, "依頼切替・日次始業"
    Exit Sub
EH:
    MsgBox "エラー: " & Err.Description, vbCritical, "依頼切替・日次始業"
End Sub
...
Private Sub ApplyFontToAllSheetCells(ByVal fontName As String, ByRef skippedOut As String)
    ...
    For Each ws In ThisWorkbook.Worksheets
        ...
        If ws.ProtectContents Then
            ...
            If ws.ProtectContents Then
                unprotectOk = False
                skippedOut = skippedOut & "・" & ws.Name & "（保護解除できずスキップ）" & vbCrLf
                Err.Clear
            End If
        End If
        ...
        If wasProt And unprotectOk Then
            ...
            End If
            If Err.Number <> 0 Then
                skippedOut = skippedOut & "・" & ws.Name & "（再保護失敗: " & Err.Description & "）" & vbCrLf
                Err.Clear
            End If
        End If
        On Error GoTo 0
...
Public Sub master_組み合わせ表を更新()
    ...
    fullRebuild = False
    If Trim$(CStr(wsOut.Cells(1, 1).Value)) <> "組合せ行ID" Then fullRebuild = True
    ...
    If fullRebuild Then
        ...
        msgDetail = "出力行数: " & CStr(totalRows) & "（全行を need / skills に基づき再生成）"
    Else
...
NextInc0:
        ...
        msgDetail = "既存の組合せ行は変更していません。" & vbCrLf & _
                    "新規の工程名+機械名グループ: " & CStr(addedPairs) & " 件を末尾に追記（データ行 +" & CStr(appendedRows) & "）。" & vbCrLf & _
                    "合計データ行数: " & CStr(totalRows)
    End If
    ...
    MsgBox "「" & SHEET_SKILL_COMBINATIONS & "」を更新しました。" & vbCrLf & msgDetail & vbCrLf & _
           "データ行は機械名→工程名→必要人数→組合せ優先度→工程+機械→組合せ行IDの順で並べ替え（上記の並びのあと、A列・組合せ行IDを上から1から振り直し）。" & vbCrLf & _
           "組合せ優先度列は列内連番（手編集可）。", vbInformation, "組み合わせ表"
    Exit Sub
...
' 工程名+機械名（列2・3）が空の行は列4「工程+機械」でグループキーにする（列1は組合せ行ID）
...
Private Sub WriteSkillCombinationSheetHeader(ByVal ws As Worksheet, ByVal maxReq As Long)
    Dim c As Long
    Dim col As Long
    ws.Cells(1, 1).Value = "組合せ行ID"
    ws.Cells(1, 2).Value = "工程名"
    ws.Cells(1, 3).Value = "機械名"
    ws.Cells(1, 4).Value = "工程+機械"
    ws.Cells(1, 5).Value = "必要人数"
    ws.Cells(1, 6).Value = "組合せ優先度"
    col = 7
    For c = 1 To maxReq
...
Private Function SkillComboFindRemarksCol(ByVal ws As Worksheet) As Long
    ...
    lastC = ws.Cells(1, ws.Columns.Count).End(xlToLeft).Column
    For c = 1 To lastC
        If Trim$(CStr(ws.Cells(1, c).Value)) = "備考" Then
...
' データ行（2行目以降）を機械名でまとまるよう並べ替え（行全体が動くため必要人数・組合せ優先度のセル値は保持される）
' キー順: 機械名→工程名→必要人数→組合せ優先度→工程+機械→組合せ行ID
...
' 並べ替え後の見た目順に合わせ、A列・組合せ行IDを 1 から連番で付け直す
...
Private Sub SkillComboEnsureMemberColumnsGrow(ByVal ws As Worksheet, ByVal maxReqOld As Long, ByVal maxReqNew As Long)
    ...
    For i = 1 To ins
        ...
        hdr = "メンバー" & CStr(maxReqOld + i)
        ws.Cells(1, remCol).Value = hdr
        remCol = remCol + 1
    Next i
End Sub

' SkillComboRowGroupKey と同じキー規則（2行ヘッダは 工程名+vbTab+機械名、1行ヘッダは列見出し文字列）
...
' skills の 1 設備列ぶんを組み合わせ表の outRow 以降に出力（組合せ行ID は sheetRowIndex を増分）
...
' -------------------------------------------------------------------------
' 設定_依頼切替前後時間 / 設定_機械_日次始業準備 … skills の工程×機械を転記して入力しやすくする
' -------------------------------------------------------------------------
...
' skills 1 行目＝工程名・2 行目＝機械名の列だけ対象（機械カレンダーと同趣旨）
...
' 依頼NO切替の準備/後始末・機械ごとの日次始業準備。
' ・シートが無ければ作成し、1 行目に見出しを書く（A1 が空のときのみ見出しを書く）。
' ・skills の 1～2 行目（工程名・機械名）から、未登録の行だけ追補する（既存行の C 列以降は変更しない）。
Public Sub MasterEnsureMachineChangeoverTemplateSheets()
    ...
    If Not ws Is Nothing Then
        If Len(Trim$(CStr(ws.Cells(1, 1).Value))) = 0 Then
            ...
            ws.Cells(1, 3).Value = "準備時間_分"
            ws.Cells(1, 4).Value = "後始末時間_分"
    ...
    If Not ws Is Nothing Then
        If Len(Trim$(CStr(ws.Cells(1, 1).Value))) = 0 Then
            ws.Cells(1, 1).Value = "機械名"
            ws.Cells(1, 2).Value = "日次始業準備_分"
            ws.Rows(1).Font.Bold = True
        End If
    End If
