...
Public Sub 設定_環境変数_シートを確保()
    ...
    Call 設定_環境変数_欠損行を試し追記(dict, ws, lastRow, "GANTT_COLOR_MODE", "", "結果_設備ガント: 空/monotone=従来淡色 full=依頼NOごとの帯色・周辺を鮮やかに")
    Call 設定_環境変数_欠損行を試し追記(dict, ws, lastRow, "EXCLUDE_RULES_TRY_OPENPYXL_SAVE", "", "互換用（Python未使用・xlwingsのみ保存）")
