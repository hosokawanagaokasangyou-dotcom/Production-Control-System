...
【配台まわりの設定の読み方】
    ...
    初回は ``pip install -r requirements.txt`` で依存を入れ、必要なら「設定_環境変数」で ``PM_AI_CMD_PAUSE_ON_ERROR`` / ``PM_AI_SETUP_COMPLETION_MSGBOX`` 等を指定する。
    ブックに ``配台_配台不要工程``
...
【外部から直接使う入口（他 .py から import される想定）】
    ...
    - ``refresh_equipment_gantt_actual_detail_only()`` … 実績明細の設備ガントシートのみ再生成。
      計画ブックの抽出日時は「加工計画DATA_実績比較用」を優先（無ければ加工計画DATA。実績明細の抽出時間は最優先）。
    ...
    - ``write_plan_actual_compare_gantt_from_snapshot_dir()`` … スナップショットの
      ``結果_タスク一覧.csv`` と現在マスタの実績明細を比較した「結果_設備ガント_計画実績比較」
      （計画／実績のほか、``TASK_INPUT_WORKBOOK`` の計画タスクシート（既定「加工計画DATA」、
      マクロ一覧からの実行時は ``COMPARE_GANTT_PLAN_TASKS_SHEET`` で「加工計画DATA_実績比較用」）に
      ``YYYY/MM/DD_加工数量`` 列がある場合は「アラジン入力数量」行を出力し、
      依頼NOごとに実績数量と突き合わせ、不一致時はセルに注記・着色）を
      ``output/plan_actual_compare_gantt.xlsx`` に出力。`plan_compare_gantt_from_snapshot.py` /
      マクロブックの「選択_計画実績比較」シート（リスト＋実行ボタン）または環境変数
      ``COMPARE_GANTT_SNAPSHOT_DIR``（未設定時はマクロブックと同一フォルダの ``pdf`` 配下の最新履歴）から呼ぶ。計画 CSV に同一機械の時間重なりがある場合は
      既定で ``PlanningValidationError``（終了コード 3）。``COMPARE_GANTT_ALLOW_PLAN_OVERLAP=1``
      のときは警告のみで生成を続行（マクロは子プロセスに同変数を付与）。
    - ``apply_result_task_column_layout_only()`` … 取り込み済み「結果_タスク一覧」の列順・非表示を
      「列設定_結果_タスク一覧」に合わせる（`apply_result_task_column_layout.py` / VBA ボタン用・openpyxl）
    - ``refresh_plan_input_dispatch_trial_order_only()`` … 「配台計画_タスク入力」の配台試行順番を
      再付与（`apply_plan_input_dispatch_trial_order.py` / openpyxl）。設定シートの行同期・分割行の自動配台不要は
    ...
    - ``refresh_dispatch_trial_pattern_list_sheet_only()`` … P1～P7 の決定論パターンを既定で一覧化
      （P3 は機械グループの納期−原反合計が短い順＋途中依頼NO優先、P4 は納期−原反日数の短い順＋途中依頼NO優先、
      P5 は P2 プローブ後に納期遅れ依頼のみ原反投入を1日前にしてから P2 を再適用、
      P6 は P5 後に再プローブし、まだ遅れの依頼のみ原反をさらに1日前にしてから P2 を再適用、
      P7 は機械名グループ＋グループ内は実効原反投入日の早い順）。
      （`apply_dispatch_trial_pattern_list_sheet.py` / openpyxl）。値は ``DISPATCH_TRIAL_PATTERN_*`` または「設定_環境変数」シート（雛形 TSV 参照）。
    - ``refresh_dispatch_trial_pattern_stage2_batch_only()`` … 各パターンで段階2を実行し、
      ...
      （`apply_dispatch_trial_pattern_stage2_batch.py` / ``xlwings_console_runner.run_dispatch_trial_pattern_stage2_batch_for_xlwings`` 等）。
    - ``refresh_dispatch_pattern_stage2_selection_to_plan_only()`` … 上記サマリの採用パターンIDに従い
      ...
      （`apply_dispatch_pattern_stage2_selection.py` / ``xlwings_console_runner.run_dispatch_pattern_stage2_selection_for_xlwings`` 等）。

【処理の流れ（ざっくり）】
    1. 段階1: 「加工計画DATA」→ 中間 `output/plan_input_tasks.xlsx`、
       マクロブック内「設定_配台不要工程」の行同期・D列→E列(Gemini)・保存（openpyxl。段階2では D→E AI なし）
    2. 段階2: master の skills / 勤怠 / 配台計画シートを読み、日付ループで割付、
       `output/` 直下に `production_plan_multi_day_*.xlsx` と `member_schedule_*.xlsx` を出力（実績明細の設備ガントのメタ日時は計画ブックの「加工計画DATA_実績比較用」を優先。実行前に同名パターンを削除・短いリトライ。
...
【依存】
    pandas, openpyxl, google.genai, cryptography（暗号化認証 JSON 利用時）
...
【初回環境】 ``pip install -r requirements.txt``（Python 3.14 以上）
"""
...
# ``pc._foo`` 等も使えるよう _core のトップレベル名をすべて公開する。
