...
- 現在の「配台計画_タスク入力」を xlwings で読み（未保存分を反映）、
  パターン P1（納期最優先）・P2（機械名+納期）・P3（納期順・機械グループの納期−原反合計が短い順・途中依頼優先）・
  P4（納期−原反日数の短い順・途中依頼優先）・P5（P2 プローブ後、納期遅れ依頼のみ原反1日前→P2 再適用）・
  P6（P5 後に再プローブ、まだ遅れの依頼のみ原反をさらに1日前→P2）を一覧化する（既定）。
- 環境変数は OS またはマクロブック「設定_環境変数」シート（A=変数名・B=値）。import 前の workbook_env_bootstrap で反映
