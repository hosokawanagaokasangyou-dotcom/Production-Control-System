...
### 3.3 段階2.1（時間外）— 残業決定後の段階2シミュレーション

| 項目 | 内容 |
|------|------|
| **UI** | 配台計画_タスク入力タブ → **段階2.1(時間外)** → 残業/休出ウィザード → 確定 → 段階2と同一の次日ダイアログ → 配台A |
| **Python** | `code/python/plan_simulation_stage2_1.py`（`PM_AI_STAGE2_1_OVERTIME=1`） |
| **配台内容** | 段階2.0 と **同一配台A**。master 勤怠を **一時上書き**（`overtime_simulation_overrides.json`）してフル配台。残業がある場合のシミュレーション。 |

**主な入力**

| 入力 | 備考 |
|------|------|
| **入力1表** | 段階2.0 と同じ。段階2.0 の事前実行は不要 |
...
1. **段階1** → 入力1表編集  
2. **段階2.0**（定時）または **段階2.1**（残業・休出を決めてから段階2を実行） → `結果_配台表.json`  
3. （任意）**段階2.5** または手動 **アラジン計画に合わせる**  
4. **手動修正** → 保存  
5. （必要時）**配台試行** — 手動表から段階2同一条件で再配台
...
*最終更新: 2026-08-25（段階2.1 は残業決定後に段階2を実行。段階2.0 事前実行は不要）*
