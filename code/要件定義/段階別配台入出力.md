...
JavaFX デスクトップが実行する **段階1〜段階3.2** および関連操作について、**何を入力に何を配台し、何が出力されるか**を整理した文書です。実装の正はソースコード（`MainShellController`、`planning_core/_core.py`、`stage2_identical_dispatch_runner.py` 等）です。
...
| 用語 | 意味 |
|------|------|
| **配台A** | `planning_core._core._generate_plan_impl()` による本配台エンジン（旧 `generate_plan`）。段階2.0 / 2.1 / 3.0〜3.2 / 配台試行が共通利用。 |
| **入力1表** | Excel シート **配台計画_タスク入力**（環境変数 `PM_AI_PLAN_INPUT_PATH` / `TASK_PLAN_SHEET`）。段階2.0・2.1 の計画タスク正本。 |
| **入力3表** | 同一ブック内シート **配台計画_タスク入力3.0**（`PM_AI_PLAN_INPUT_STAGE3_SHEET`）。枝番分解後のタスクのみ。段階3.0〜3.2 の入力。 |
| **結果_配台表** | 配台成果の JSON（`結果_配台表.json`）および同名 Excel シート。タスク×配台日×数量のワイド表の正本。 |
| **正本 output** | 既定 `{リポジトリ}/output/`（`PM_AI_OUTPUT_DIR` で上書き可）。 |

### 1.1 段階番号の整理（混同注意）

UI 上 **「段階3」** という名称が **2 系統** 存在します。

| 呼称（本書） | 旧 UI 名 | 実行入口 | 概要 |
|--------------|----------|----------|------|
| **配台試行** | 段階3（配台計画手動修正・ボタン非表示） | `dispatch_interactive_trial.py` | 手動修正済み `結果_配台表.json` をマージし、段階2と同一条件で再配台。 |
| **段階3.0 / 3.1 / 3.2** | 入力3表タブの段階3.x | `plan_simulation_stage3_0.py` 等 | **入力3表（枝番）** で配台A → **枝番統合**して正本 JSON へ反映。 |
...
flowchart TD
  ...
  DI --> Gen3[入力3表を生成 build_stage3_input.py]
  Gen3 --> In3[入力3表 枝番タスク]
  In3 --> S30[段階3.0 / 3.1 / 3.2]
  S30 --> RD2[結果_配台表 枝番統合後]
  In1 --> S21[段階2.1 時間外]
  S21 --> RD
  RD --> S25[段階2.5 AI 整列]
...
| ボタン | 処理 |
|--------|------|
| **アラジン計画に合わせる** | Java `DispatchAladdinPlanAligner`：翌日以降を (アラ計画) に沿い **ロール単位**で日間再配分。`shaped_aladdin_plan.json` 参照。 |
| **入力3表を生成** | `build_stage3_input.py` → 入力3表シートへ枝番行書込（後述） |
| **配台リプレイ** | 配台完了後の可視化（再配台ではない） |

---

### 3.6 入力3表生成 — 段階3.0 の前処理

| 項目 | 内容 |
|------|------|
| **UI** | 配台計画手動修正 → **入力3表を生成** |
| **Python** | `code/python/build_stage3_input.py` → `stage3_input_builder.py` |
| **配台内容** | **配台しない**。手動修正 JSON を **枝番タスク** に分解して Excel へ書く。 |

**主な入力**

| 入力 | 備考 |
|------|------|
| **`結果_配台表.json`** | 保存済み（依頼NO×工程×機械×配台日×当日配台数量） |
| **入力1表ブック** | `PM_AI_PLAN_INPUT_PATH`（元行の複製元） |

**主な出力**

| 出力 | 備考 |
|------|------|
| **シート「配台計画_タスク入力3.0」** | 枝番依頼NO `{元}-{2桁}`、列 **元依頼NO**・**配台枝番**、原反投入日=配台日、配台可能日時=12:45 / 上書き=定常開始 |
| **入力3表タブ** | Java が Spreadsheet UI で表示・編集 |

**ルール**: 元タスク行は入力3表に含めない。特別ルール scope は **元依頼NO** で親に紐付け。

---

### 3.7 段階3.0 — 枝番タスク配台 + 統合

| 項目 | 内容 |
|------|------|
| **UI** | 配台計画_タスク入力3.0 タブ → **段階3.0 実行** |
| **Python** | `code/python/plan_simulation_stage3_0.py` → `run_stage3_generate_plan(qty_strict=False)` |
| **配台内容** | **入力3表のみ**を `tasks_df_override` として **配台A** 実行 → **`stage3_branch_merge`** で枝番を **元依頼NO** に統合し正本 JSON 更新。 |

**主な入力**

| 入力 | 備考 |
|------|------|
| **入力3表** | `PM_AI_PLAN_INPUT_PATH` + シート `配台計画_タスク入力3.0` |
| master / ルール / ルックアップ | 段階2 と同様 |
| env | `PM_AI_PLAN_INPUT_STAGE3=1`（runner が設定） |

**主な出力**

| 出力 | 備考 |
|------|------|
| **`結果_配台表.json`** | 枝番配台結果 → **統合後**が正本 |
| xlsx / production_plan / member_schedule | 段階2 と同様に生成 |
| ログ | `[stage3-merge] 枝番統合: N 行 → M 行` |

---

### 3.8 段階3.1（時間外）— 入力3表 + 残業

| 項目 | 内容 |
|------|------|
| **UI** | 入力3表タブ → **段階3.1(時間外)** → ウィザード → 確定 |
| **Python** | `code/python/plan_simulation_stage3_1.py`（`PM_AI_STAGE2_1_OVERTIME=1` + 残業 JSON） |
| **配台内容** | 段階3.0 と同じ **入力3表 + 配台A + 枝番統合**。勤怠のみ段階2.1 と同じ hybrid 上書き。 |

**入出力**は段階3.0 に加え **`overtime_simulation_overrides.json`** のみ差分。実行後 JSON 削除（書き戻し相当）。
...
| 項目 | 内容 |
|------|------|
| **UI** | 入力3表タブ → **段階3.2(数量厳守)** |
| **Python** | `code/python/plan_simulation_stage3_2.py` → `run_stage3_generate_plan(qty_strict=True)` |
| **配台内容** | 入力3表 + 配台A。**同日完走必須**・**定常外人ブロック無視**（`PM_AI_STAGE3_2_QTY_STRICT=1`）→ 枝番統合。 |

**入出力**は段階3.0 と同一パス。`_generate_plan_impl` 内分岐のみ異なる。

---

### 3.10 配台試行（旧「段階3」ボタン・インタラクティブ）
...
| UI 表示 | (段階3前)/(段階3後)/(段階3改) 行ラベル |

**段階3.0 系との違い**: 枝番分解・入力3表・枝番統合 **なし**。手動修正表を **そのまま** 配台目標にマージする試行。
...
| 列（代表） | 意味 |
|------------|------|
| 依頼NO / タスクID | タスク識別（枝番時は `12345-01` 形式） |
| 工程 / 機械名 | 割付先 |
| **配台日** | 暦日 |
| **当日配台数量** | その日に配台する長さ (m)。手動修正・段階3前表示の正 |
| 実配台数量 | 段階3 **配台試行** 後にタイムライン実績が載る列（試行後 UI） |
...
| 変数 | 段階 | 意味 |
|------|------|------|
| `PM_AI_PLAN_INPUT_PATH` | 2〜3 | 計画入力 Excel パス |
| `PM_AI_OUTPUT_DIR` | 全般 | 出力ディレクトリ（既定 `output/`） |
| `PM_AI_MASTER_WORKBOOK` | 1〜3 | master.xlsm |
| `PM_AI_STAGE2_SKIP_TODAY_DISPATCH` | 2.0 | 1=当日配台しない |
| `PM_AI_STAGE2_1_OVERTIME` | 2.1 / 3.1 | 1=残業シミュ有効 |
| `PM_AI_OVERTIME_SIMULATION_JSON` | 2.1 / 3.1 | 残業 overrides パス |
| `PM_AI_PLAN_INPUT_STAGE3` | 3.0〜 | runner が 1 に（入力3表読込） |
| `PM_AI_STAGE3_2_QTY_STRICT` | 3.2 | 1=数量厳守モード |
...
5. **手動修正** → 保存  
6. **入力3表を生成** → 入力3表確認・編集  
7. **段階3.0**（必要なら **3.1** 時間外 / **3.2** 数量厳守）  
8. （別系統）**配台試行** — 入力3 パイプラインを使わず手動表から再配台する場合
...
| 段階2/3 ランナー | `code/python/planning_core/stage2_identical_dispatch_runner.py` |
| 入力3 生成 | `code/python/planning_core/stage3_input_builder.py` |
| 枝番統合 | `code/python/planning_core/stage3_branch_merge.py` |
| 業務ルール文章 | `code/要件定義/配台ルール.md` |

---

*最終更新: 2026-06-01（リポジトリ実装に基づく）*
