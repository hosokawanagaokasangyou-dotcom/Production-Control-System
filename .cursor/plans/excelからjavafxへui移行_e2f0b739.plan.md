...
todos:
  ...
  - id: decision-8-dual-running
    ...
    status: pending
  - id: plan-l0-l8-workstream-note
    content: 別紙「不足取り決め追記」プランの bootstrap-toolchain-decisions／python-bridge-protocol 等 9 項目は、本 YAML の decision-l0-toolchain-finalize〜decision-8-dual-running および bootstrap-javafx 等とスコープ統合済み（二重 pending は置かない）
    status: completed
isProject: false
---
...
4. **Python ETL 実装**を着手する際は、**§実装前に固める取り決め**の **L3（§3）** および **`fetch_latest_files`** 方針、`powerquery-etl-parity`／`pq-m-export-from-workbook` の TODO と整合させる。
...
以下と **§実装前に固める取り決め（決定値）**（**L0〜L8 ナビ**）を往復参照する。**確定方針は常に後者を正**とする。

**ギャップ項目と取り決め（L）の対応**: **1** xlwings → **L1（§1）** IPC・単一実行・終了コード。**2** マクロブック分解 → **L2（§2）** 設定・パス・**L8（§8）** 運用。**3** インストール → **L0** ツールチェーン＋**L5（§5）** ZIP／venv。**4** 複数 PQ → **L3（§3）** Python ETL・`fetch_latest_files`。**5** 名前定義 PQ → **L2（§2）** `settings.json`／出力パス。**6** 選択的 Java 化 → 本文フェーズ 6 および概要のハイブリッド方針。
...
**L0〜L8 ナビ（本節内のラベル対応）**: **L0**＝直下「### L0. 基盤・ツールチェーン」。**L1**＝「### 1. Python ↔ …」。**L2**＝「### 2. データコントラクト…」。**L3**＝「### 3. PQ 代替 ETL…」。**L4**＝「### 4. 画面…」。**L5**＝「### 5. 配布…」。**L6**＝「### 6. 品質保証…」。**L7**＝「### 7. ドキュメント…」。**L8**＝「### 8. 失われやすい運用…」。§「Excel との論理的切り離し」は **L0・L8 と整合する補助節**（業務ロジックと成果物形式の境界）。
...
> **更新**: L0 と 1〜8 の決定値は **§ 実装前に固める取り決め（決定値）** に確定済み。下表は **論点の歴史記録**として残す。最新の正は上記節および **L0〜L8 ナビ表**を参照。

確定した運用・技術の値は、本文の **§実装前に固める取り決め（決定値）** および YAML フロントマターの **`decision-l0-toolchain-finalize`** ～ **`decision-8-dual-running`**（従来の `bootstrap-javafx` 等とあわせて）をワーク分解の起点とする。
