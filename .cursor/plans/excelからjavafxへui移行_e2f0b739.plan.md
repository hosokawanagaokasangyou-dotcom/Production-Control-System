...
| **8.2 マスタブック（`master.xlsm`）等** | **マスタはファイルとして保持**するが、**更新経路を Excel 専用に固定しない**。当面 Excel で編集してもよいが、**ロジックは常にファイル読込のみ**とし、マクロ実行を前提にしない。**将来的な JavaFX 側マスタ編集 UI は別判断**（工数と優先度でスコープ化）。 |
