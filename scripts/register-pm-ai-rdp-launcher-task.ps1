...
if ([string]::IsNullOrWhiteSpace($IniPath)) {
  # PM-AI が書き込む ini は配備先フォルダ（PmAiRdpRemoteLauncher.exe と同階層）の RPA設定.ini。
  # portable 等 exe だけ別フォルダのときは -IniPath で配備先フォルダを明示指定すること。
  $IniPath = Join-Path $launcherDir 'RPA設定.ini'
}
...
Write-Host "  Log (UNC): $launcherDir\launcher-logs\launcher-yyyyMMdd.log"
