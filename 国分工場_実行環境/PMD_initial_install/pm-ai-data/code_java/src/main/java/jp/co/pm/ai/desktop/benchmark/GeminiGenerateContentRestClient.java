...
public final class GeminiGenerateContentRestClient {
    ...
    /**
     * {@link #generateContent} と同様だが、応答本文を切り捨てずに返す（AI 応答の JSON 解析用）。
     */
    public record FullBodyResult(int httpStatus, long wallTimeNanos, String body, String errorSummary) {
    ...
    /**
     * Gemini {@code generateContent} の HTTP 本文をそのまま返す。
     *
     * @see #extractFirstCandidateText(String)
     */
    public static FullBodyResult generateContentFullBody(
            String apiKey,
            String modelId,
            String userPrompt,
            int maxOutputTokens,
        ...
        return new FullBodyResult(code, dt, body, err);
    }

    /**
     * {@code generateContent} の JSON 応答から、先頭候補のテキスト部分を連結して返す。
     */
    public static Optional<String> extractFirstCandidateText(String responseBody) throws IOException {
