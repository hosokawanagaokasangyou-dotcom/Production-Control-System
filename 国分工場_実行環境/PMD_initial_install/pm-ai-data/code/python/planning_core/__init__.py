...
【外部から直接使う入口（他 .py から import される想定）】
    ...
    - ``generate_plan()`` … 段階2。実体は ``stage2_identical_dispatch_runner.run_stage2_generate_plan``（`plan_simulation_stage2.py` から呼ぶ）
...
【依存】
    ...
    API キーは GEMINI_CREDENTIALS_JSON 等で解決する証明書 JSON（および plan_input／workspace 同階層の既定ファイル）からのみ。
    暗号化 JSON の復号は ``planning_core`` 内の定数のみ使用（値はソースにあり、社内共有資料やログには書かないこと）。ログにキーは出さない。

【初回環境】 テストコード直下で ``py -3.14 参照用/python/setup_environment.py`` またはマクロ「環境構築」（requirements.txt 一括）
"""
