...
| 操作者 | 保存・閲覧とも `OperatorUserPaths.resolveOperatorUser` / セッション名を既存操作ログ・世代と同じ規約で解決。閲覧コンボの既定はログイン中セッションを優先 |
| 保持 | 操作者あたりタイムスタンプフォルダ名の **最新 20 件**（フォルダ名の古い順に削除。mtime 非依存。アラジン入力 Excel 世代と同数） |
...
※ 同一化チェックの比較（シス計のみ突合・完了行除外）は本機能の実装範囲に含む。
...
4. 失敗してもチェック結果ダイアログ表示は阻害しない（保存は `Optional.empty()`。終了ゲートの再比較は `persistHistory=false` で履歴を増やさない）
