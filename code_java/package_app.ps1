...
function Build-PmAiDesktopLauncherBatContent {
    ...
    $modParts = foreach ($a in $artifacts) {
        '%ROOT%\app\' + $a + '-' + $JavafxVersion + '-win.jar'
    }
    $modPathValue = [string]::Join(';', $modParts)
    $lines = @(
        '@echo off',
        'rem ASCII-only. Generated by package_app.ps1 from pom javafx.version.',
        ...
        ':have_java',
        '',
        'set "PM_AI_JFX_MODPATH=' + $modPathValue + '"',
        '',
        '"%JAVA_EXE%" -Dfile.encoding=UTF-8 -Xms' + $JvmInitial + ' -Xmx' + $JvmMax + ' -XX:+HeapDumpOnOutOfMemoryError -XX:+UseStringDeduplication -Dprism.order=sw --module-path "%PM_AI_JFX_MODPATH%" --add-modules javafx.controls,javafx.fxml,javafx.graphics,javafx.base,javafx.swing -classpath "%ROOT%\app\*" jp.co.pm.ai.desktop.PmAiFxApp %*',
        '',
