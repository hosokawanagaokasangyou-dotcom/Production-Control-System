...
public final class ConflictSaveDialog {
    ...
    public static ConflictSaveChoice show(
        ...
        String who = screenTitle != null && !screenTitle.isBlank() ? screenTitle : "保存先";
        if (overwriteOrCancelOnly) {
            dialog.setHeaderText(who + " を、画面の内容で上書きしますか？");
            dialog.getDialogPane().setContent(null);
        } else {
            dialog.setHeaderText(who + " を保存する前に、ファイルの違いを確認してください");
