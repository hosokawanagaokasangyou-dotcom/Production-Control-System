...
public final class EnvVarDocs {
    ...
    static {
        ...
        put(
                ...
                "後加工検査表フォルダ（フルパス）。工場ルート（湖南: Box\\長岡産業\\後加工検査表\\湖南工場、"
                        + "国分: 同\\国分工場）。配下の年・工程サブフォルダを再帰走査する。"
                        + "初期値は空。未設定時の実行時解決は %USERPROFILE%\\Box\\長岡産業\\後加工検査表\\{工場}。"
                        + "起動時・環境初期化・工場切替で未設定ならフォルダ選択を案内（確認入力 kensa のあと）。"
                        + "環境変数タブのフォルダ選択時も kensa 入力が必要。"
