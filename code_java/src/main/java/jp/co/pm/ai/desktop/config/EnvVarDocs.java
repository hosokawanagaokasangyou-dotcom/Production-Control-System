...
public final class EnvVarDocs {
    ...
    static {
        ...
        put(
                ...
                "アラジンへ入力された加工計画を Power Query で吸出し表示する"
                        + "「新 マシン別納期管理表.xlsm」のフルパス。"
                        + "実行・ログタブから読み取り専用で開く。"
                        + "空のときは工場既定（湖南: 生産管理システム/アラジンオフィスシステムデータ/納期管理/新 マシン別納期管理表.xlsm。"
                        + "国分: 空。実行・ログの「マシン別納期管理表を開く」は湖南のみ表示）。"
                        + "環境変数初期化の工場選択でも設定。"
                        + "環境変数タブのファイル選択可。");
        put(
        ...
        put(
                "PM_AI_INSPECTION_SHEET_DIR",
                "後加工検査表フォルダ（フルパス）。"
                        + "空のとき湖南工場既定 UNC（生産管理システム/アラジンオフィスシステムデータ/003 検査表）。"
                        + "湖南は当該 UNC に加え Box\\長岡産業\\後加工検査表\\湖南工場も走査する。"
                        + "国分は %USERPROFILE%\\Box\\長岡産業\\後加工検査表\\国分工場。"
                        + "配下の年・工程サブフォルダを再帰走査する。"
                        + "起動時にフォルダ選択ダイアログは出さない。"
                        + "環境変数タブのフォルダ選択時は kensa 入力が必要。"
                        + "フォルダ配下は読取専用。索引 CSV はユーザーホームへ保存。"
                        + "共有配布は PM_AI_INSPECTION_SHEET_INDEX_SHARE_DIR。");
        put(
                "PM_AI_INSPECTION_SHEET_INDEX_SHARE_DIR",
                "検査表索引 CSV の共有配布フォルダ（フルパス）。"
