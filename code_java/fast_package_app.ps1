...
function Copy-WorkspaceTreeWithExplicitExclusions {
    ...
    # traversal so we never recurse into .git/ or huge Cash_PMD/ trees.
    # Fast variant naming: cache = Cash_PMD/, dist = PMD_fast/ (vs package_app.ps1: build_cache/, dist/).
    param(
        [string]$RepoRoot,
        [string]$DestRoot
    )
    ...
    # Directory base names matched anywhere in the path (caches / build outputs / plan artifacts).
    # Exact match by design: 'plan' will NOT match 'planning_core' (Python core stays bundled).
    $excludedDirNames = @(
        '__pycache__',
        '.pytest_cache',
        'build_cache',
...
function Copy-BundleToDist {
    ...
    @(
        ...
        'Workspace mirror source: filesystem walk with explicit exclude list in fast_package_app.ps1.',
        'Master *.txt under code/ are always copied (see package_app_mandatory_code_paths.txt).',
        'Excluded dirs: .git, .venv, code_java/{target,package_input,PMD_fast,output,Cash_PMD}, **/build_cache, **/plan, **/plans, **/__pycache__, **/.pytest_cache.',
        ...
        'Python: pm-ai-data\runtime\python-embed\python.exe (from Cash_PMD embed + pip).',
...
$cacheRoot = Join-Path $Root 'Cash_PMD'
...
# Final output always under code_java\PMD_fast. jpackage --dest may use a staging folder: non-ASCII paths
# can produce runtime\bin with DLLs but no java.exe on some JDK builds.
$distFinal = Join-Path $Root 'PMD_fast'
...
# When staging to TEMP, do not pre-delete code_java\PMD_fast: Explorer / running exe often locks it.
# jpackage only writes to $jpkgDestParent; we replace PMD_fast\<APP_NAME> after success.
    ...
    if (Test-Path -LiteralPath $destAppPath) {
        ...
        for ($ri = 0; $ri -lt 8; $ri++) {
            try {
                ...
                Write-Warning "Cannot remove old bundle folder (close $($APP_NAME).exe / Explorer on PMD_fast\$APP_NAME). Retry ($ri/8)..."
...
Common causes:
  ...
  3) Stale PMD_fast - ensure code_java\PMD_fast was removed before jpackage.
...
if ($PackageType -ne 'app-image') {
    Write-Host 'Check PMD_fast for installer output.'
}
if (-not $WinConsole -and $PackageType -eq 'app-image') {
    Write-Host 'Hint: console build: .\fast_package_app.ps1 -WinConsole' -ForegroundColor DarkGray
}